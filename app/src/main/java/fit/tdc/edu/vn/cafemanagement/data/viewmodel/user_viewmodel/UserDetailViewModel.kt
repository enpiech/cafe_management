...
class UserDetailViewModel(
    ...
    val currentStorePos = CombinedLiveData(currentStore, storeList) { store, storeList ->
        getCurrentStorePos(store, storeList)
    }

    private fun getCurrentStoreOfUser(user: User?, res: List<Store>?): Store? {
        if (!res.isNullOrEmpty() && user != null) {
            return res.find { store -> store.id == user.storeId }
        }
        return null
    }

    private fun getCurrentStorePos(store: Store?, res: List<Store>?): Int {
        if (!res.isNullOrEmpty() && store != null) {
            return res.indexOf(store)
        }
        return 0
    }
    ...
    override fun validate(item: User?) {
        ...
        Log.d(
            "test",
            item?.storeId.toString() + ":" + currentItem.value!!.storeId.toString() + formState.isChanged
        )
